...
// --- FERRAMENTAS DE TESTE (DESENVOLVEDOR) ---
let modoDeus = false;
let modoAutomatico = false;
let modoExtremo = false; // Novo botão de fast-forward!
...
document.addEventListener('keydown', (evento) => {
    ...
    // Novo: Tecla H para velocidade extrema (Fast-forward)
    if (evento.key.toLowerCase() === 'h') {
        modoExtremo = !modoExtremo;
        clearInterval(cicloJogo);
        
        if (modoExtremo) {
            cicloJogo = setInterval(atualizarJogo, 20); // Rodando a absurdos 20ms
        } else {
            // Se desligar, volta pra velocidade em que estava (lento ou normal)
...
function atualizarJogo() {
    ...
    // 4. PEGAR ORBE DO TEMPO (Agora dura apenas 5 segundos!)
    if (orbTempo.ativo && novaCabeca.x === orbTempo.x && novaCabeca.y === orbTempo.y) {
        tocarSom(somPowerUp); 
        orbTempo.ativo = false; 
        lentidaoAtiva = true;

        // Se o modo extremo não estiver ativado, deixa lento. Se estiver extremo, ignora a lentidão visualmente
        if (!modoExtremo) {
            clearInterval(cicloJogo);
        ...
        // Timeout alterado para 5.000 milissegundos (5 segundos)
...
function desenharJogo() {
    ...
    // Alerta de modo extremo
    if (modoExtremo) {
        ctx.fillStyle = "red";
        ctx.fillText("⚡ MODO EXTREMO (H)", 20, linhaTextoY);
        linhaTextoY += 30;
    }

    if (lentidaoAtiva) {
        ctx.fillStyle = "#00ffff";
        ctx.fillText("❄️ TEMPO LENTO (5s) ❄️", 20, linhaTextoY);
        linhaTextoY += 30;
    }
...
function iniciar() {
    ...
    if (imagensCarregadas === totalImagens) {
        ...
        if (escolhaVelocidade === '1') velocidadeOriginal = 300; 
        else if (escolhaVelocidade === '3') velocidadeOriginal = 80;  
        else velocidadeOriginal = 150; 
